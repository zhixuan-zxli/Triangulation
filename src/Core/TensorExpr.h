...
{
  assert((VecCompare<int,Dim>().compare(size(), expr.box().size())) == 0);
...
#pragma omp parallel for default(shared) schedule (static) reduction(max:r)
    loop_sz_1(sz, i)
      r = std::max(r, expr.at(i));
  } else if(Dim == 2) {
#pragma omp parallel for default(shared) schedule (static) reduction(max:r)
    loop_sz_2(sz, i, j)
      r = std::max(r, expr.at(i, j));
  } else if(Dim == 3) {
#pragma omp parallel for default(shared) schedule (static) reduction(max:r)
    loop_sz_3(sz, i, j, k)
      r = std::max(r, expr.at(i, j, k));
  } else if(Dim == 4) {
#pragma omp parallel for default(shared) schedule (static) reduction(max:r)
    loop_sz_4(sz, i, j, k, l)
      r = std::max(r, expr.at(i, j, k, l));
  }
  return r;
}
